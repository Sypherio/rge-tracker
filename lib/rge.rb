...
module RGE
  class TrendFinder
    ...
    RGE_SITE = 'http://ebiz1.rge.com/OutageReports/RGEMONROE.html'
    LOG_NAME = 'rge_logs'
    ...
    def find_outages
      ...
      prev_outages = 0
      while true
        total_customers = mechanize.get(RGE_SITE).body.split('service to ')[1].split(' ')[0].gsub(',','').to_f
        curr_outages = mechanize.get(RGE_SITE).body.split('&nbsp;</th><th>&nbsp;</th><th>')[1].split('</th></tr>')[0].gsub(',','').to_f
    ...
    def write_outages(curr_outages)
      `touch #{LOG_NAME}` unless File.exists?(LOG_NAME)
